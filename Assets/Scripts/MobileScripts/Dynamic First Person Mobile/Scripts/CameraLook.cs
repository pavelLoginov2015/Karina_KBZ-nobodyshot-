...
namespace FirstPersonMobileTools.DynamicFirstPerson
{
	...
	public class CameraLook : MonoBehaviour {	
		...
		private Func<ControlFreak2.InputRig.Touch, bool> m_IsTouchAvailable;						// Delegate takes parameter touch and return true if touch is the available touch for camera rotation
		...
		private void Update() 
		{
			...
			if (ControlFreak2.CF2Input.touchCount == 0) return;
		...
		public void OnChangeSettings()
		{
			...
			switch (m_TouchDetectMode)
			{
				case TouchDetectMode.FirstTouch: 
					m_IsTouchAvailable = (ControlFreak2.InputRig.Touch touch) => { return touch.fingerId.ToString() == m_AvailableTouchesId[0]; }; 
					break;
				case TouchDetectMode.LastTouch:
					m_IsTouchAvailable = (ControlFreak2.InputRig.Touch touch) => { return touch.fingerId.ToString() == m_AvailableTouchesId[m_AvailableTouchesId.Count - 1]; };
					break;
				case TouchDetectMode.All:
					m_IsTouchAvailable = (ControlFreak2.InputRig.Touch touch) => { return m_AvailableTouchesId.Contains(touch.fingerId.ToString()); };
					break;
			}
