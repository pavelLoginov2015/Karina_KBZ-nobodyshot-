...
using UnityEngine;
using kube;
public class TeamInfectionStart : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (_teamAvail[i])
            {
                num2 = Mathf.Max(Kube.BCS.playersInTeam[i], num2);
                num3 = Mathf.Min(Kube.BCS.playersInTeam[i], num3);
            }
        }
        bool flag = false;
        if (Mathf.Abs(num2 - num3) > 2)
        {
            flag = true;
        }
        for (int j = 0; j < 2; j++)
