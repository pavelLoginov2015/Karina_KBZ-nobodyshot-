...
public class TankScript : TransportScript
{
	...
	private void DriveCar0()
	{
		...
		{
			wheelsPhys[num2].transform.localRotation = Quaternion.Euler(0f, wheelMaxRotateAngle[num2] * ruleRotate, 0f);
		}
		...
		float num3 = driverCameraToMove[0].transform.rotation.eulerAngles.z - ControlFreak2.CF2Input.GetAxis("Mouse Y") * 4f;
		if (num3 > 180f)
		{
			num3 -= 360f;
		}
	...
	public override void TransportLateUpdate(int numPlace)
	{
		...
		case 0:
		{
			...
			Vector3 vector2 = base.transform.InverseTransformDirection(vector - canonTower.transform.position);
			canonGunRotation.x = (0f - Mathf.Atan2(vector2.z, vector2.x)) * 57.29578f;
			canonGunRotation.y = Mathf.Atan2(vector2.y, Mathf.Sqrt(vector2.x * vector2.x + vector2.z * vector2.z)) * 57.29578f;
			break;
		}
		}
	}
	...
	private void _CreateCanonShot(Vector3 rayOrigin, Vector3 rayDirection, PhotonMessageInfo info)
	{
		...
		canonGun.GetComponent<AudioSource>().Play();
	}
