...
public class CaptureTheFlagController : TeamControllerBase
{
	...
	private void Start()
	{
		...
		if (respawnsNums == 0){
		    Kube.BCS.ExitGame();
			Kube.GPS.printMessage("На карте нет респаунов!",Color.yellow);
		}
	}
	...
	private bool CheckFlagsLoaded()
	{
		...
		if (array.Length > 0)
		{
			flags = new FlagScript[array.Length];
			for (int i = 0; i < flags.Length; i++)
			{
				flags[i] = array[i].GetComponent<FlagScript>();
			}
		}
		if (flags != null)
		{
	...
	public void FlagCaptured(int playerCaptured, int teamCaptured, int teamLose)
	{
		...
		Object.Instantiate(Kube.ASS4.soundFlagCaptured, Vector3.zero, Quaternion.identity);
	}
}
