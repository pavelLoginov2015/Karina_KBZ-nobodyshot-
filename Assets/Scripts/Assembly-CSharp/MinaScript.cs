...
public class MinaScript : MonoBehaviourPun
{
	...
	private void Start()
	{
		...
		activeStartTime = Time.time + activationTime;
	}

	private void Update()
	{
		if (base.photonView.IsMine && Time.time > activeStartTime && !startExp && Time.time - lastCheckTime > checkDeltaTime)
		{
			GameObject[] array = GameObject.FindGameObjectsWithTag("Player");
			for (int i = 0; i < array.Length; i++)
			{
				PlayerScript component = array[i].GetComponent<PlayerScript>();
				if (!component.dead && Vector3.Distance(base.transform.position, array[i].transform.position) < pickpickDist)
				{
					startExp = true;
					StartExplosion();
					break;
			...
			{
				if (Vector3.Distance(base.transform.position, array2[j].transform.position) < pickpickDist)
				{
					startExp = true;
					StartExplosion();
					break;
			...
			lastCheckTime = Time.time + checkDeltaTime;
		}
		if (startExp)
		{
			GetComponent<Renderer>().material.color = new Color(0.6f + 0.2f * Mathf.Sin(Time.time * 30f), 0.6f + 0.2f * Mathf.Sin(Time.time * 30f), 0.6f + 0.2f * Mathf.Sin(Time.time * 30f), 0f);
	...
	private void StartExplosion()
	{
		...
		{
			base.photonView.RPC("_StartExplosion",RpcTarget.All);
		}
	}
	...
	private void _StartExplosion(PhotonMessageInfo info)
	{
		...
		timeToExplosion = Time.time + pickpickTime;
	}
